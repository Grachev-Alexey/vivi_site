  ...
  return (
    <>
      ...
      <div className={`fixed top-0 right-0 h-screen w-full max-w-md bg-white z-[101] shadow-2xl p-8 pt-20 flex flex-col transition-transform duration-300 ease-out overflow-y-auto ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <div className="flex flex-wrap gap-2">
            {CITIES.map(city => (
              <button 
                ...
                className={`px-4 py-2 rounded-xl text-sm font-bold transition-colors duration-200 ${city.id === selectedCity.id ? 'bg-brand-500 text-white' : 'bg-gray-100 text-dark hover:bg-brand-50'}`}
              >
                {city.name}
              </button>
            ))}
        ...
        <nav className="flex flex-col gap-3 border-t border-gray-100 pt-6">
